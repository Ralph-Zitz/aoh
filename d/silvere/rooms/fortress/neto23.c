...
varargs void create()
{
   ::create();
   Set(P_INDOORS,1);
   Set(P_INT_SHORT,"Entrance the northeastern tower");
   Set(P_INT_LONG,
    ...
    "other one to the eastern battlement. There's also a staircase leading "
    "up to the top of the tower and down to the tower lower level.\n");

   Set(P_INT_WIZMSG,
    " South: Enter eastern battlement.\n"
    " East:  SE Corner of NE tower, level 2.\n"
    " North: NE Corner of NE tower, level 2.\n"
    " West:  Enter northern battlement.\n"
    " Up:    Top of the NE tower, level 3.\n"
    " Down:  Bottom of the NE tower, level 1.\n");
...
//   Set(P_COORDINATES, ({ ({ -50,20,0 }), C_SILVERE }) );
   ...
   AddExit("north",  SIL_FORTRESS("neto21"));
   AddExit("south",  SIL_FORTRESS("eabat1"));
   AddExit("east",   SIL_FORTRESS("neto24"));
   AddExit("west",   SIL_FORTRESS("nobat9"));
   AddExit("up",     SIL_FORTRESS("neto33"));
   AddExit("down",   SIL_FORTRESS("neto13"));
