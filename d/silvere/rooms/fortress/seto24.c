...
string CheckFields()
{
   ...
   switch(({int})NIGHTDAY->QueryState())
   {
      ...
      case ND_PREDAWN..ND_SUNRISE:
         desc+=
          "to the east you can see the fields dimly ligthed while "
          "the fiery rays of the sun illuminate the mountain peaks "
          "making them look aflame. To the south, the first light of "
          "the day gives the river a shiny hue and forces the shadows "
          "from the fields to retreat.\n";
         break;
      case ND_TWILIGHT..ND_EVENING:
         desc+=
          "to the south the shadows grow longer in the diminishing "
          "light of the day on the river and the fields while to the "
          "east the shadow of the fortress extends across the fields "
          "below and the mountains in the distance are bathed with "
          "the last light of the day.\n";
         break;
      case ND_EARLY_MORNING..ND_SUNSET:
         desc+=
          "to the south the you can see fields stretching to the river "
          "that shines under the bright light of the sun, while looking "
          "east you can see fields with the snowy peaks of the "
...
varargs void create()
{
   ::create();
   Set(P_INDOORS,1);
   Set(P_INT_SHORT,"Northeastern corner from northeastern tower");
   Set(P_INT_LONG,
    ...
    "filled with arrows next to the wall. A bit to the northwest lies a "
    "staircase that blocks the view of the tower entrance.\n");

   Set(P_INT_WIZMSG,
    " North: NE Corner of SE tower, level 2.\n"
    " West:  SW Corner of SE tower, level 2.\n");
   ...
   AddDetail( ({"wall","walls"}),
    ...
    "northwest the staircase blocks view of the doorway while around you "
    "there are several arrow slits.\n");
   ...
   AddDetail( ({"stairs","staircase"}),
    "A narrow staircase opens a bit to the southwest, it looks like it leads "
    "up and down.\n");
...
//   Set(P_COORDINATES, ({ ({ -50,20,0 }), C_SILVERE }) );
   ...
   AddExit("north",  SIL_FORTRESS("seto22"));
   AddExit("west",   SIL_FORTRESS("seto23"));
