   ...
   switch(({int})NIGHTDAY->QueryState())
   {
      ...
      case ND_EARLY_MORNING..ND_SUNSET:
         desc+=
          ...
          "fortress and the western battlement.\n";
         break;
      default:
         desc+=
...
string CheckFields()
{
   ...
   switch(({int})NIGHTDAY->QueryState())
   {
      ...
      case ND_PREDAWN..ND_SUNRISE:
         desc+=
          "you can see the fields are still shrouded in darkness "
          "but the first rays of the sun illuminate the green "
          "shadow over the hills in the distance.\n";
         break;
      case ND_TWILIGHT..ND_EVENING:
         desc+=
          "the last light of the day shines over the fields and "
          "the green shadow that marks the edge of the forest, "
          "crowning the rolling hills far away into the distance.\n";
         break;
      case ND_EARLY_MORNING..ND_SUNSET:
         desc+=
          "you can see fields and a green shadow crowning the "
          "rolling hills, marking the edge of the forest far "
          "into the distance.\n";
         break;
      default:
         desc+=
...
string CheckBatt()
{
   ...
   desc=
    "Looking north from the arrow slits ";
   switch(({int})NIGHTDAY->QueryState())
   {
      case ND_NIGHT:
         desc+=
          "you can barely make out the shapes of the western "
      ...
      case ND_PREDAWN..ND_SUNRISE:
         desc+=
          ...
          "the top of the battlements. The rest is still in darkness.\n";
         break;
      case ND_TWILIGHT..ND_EVENING:
         desc+=
          "you see last light of the day illuminating the battlements as the "
          "night approaches.\n";
         break;
      case ND_EARLY_MORNING..ND_SUNSET:
         desc+=
          "the west battlements can be seen, they are made from stone and "
...
varargs void create()
{
   ::create();
   Set(P_INDOORS,1);
   Set(P_INT_SHORT,"Northwestern corner of southwestern tower");
   Set(P_INT_LONG,
    ...
    "filled with arrows next to the wall. A bit to the east lie the "
    "entrance to the tower and a staircase.\n");

   Set(P_INT_WIZMSG,
    " South: SW Corner of SW tower, level 2.\n"
    " East:  Entrance to the tower, level 2.\n");
   ...
   AddDetail( ({"wall","walls"}),
    ...
    "east there are doorways to the battlements while around you there "
   ...
   AddDetail( ({"stairs","staircase"}),
    "A narrow staircase opens a bit to the east, leading up and down.\n");
...
//   Set(P_COORDINATES, ({ ({ -50,20,0 }), C_SILVERE }) );
   ...
   AddExit("south",  SIL_FORTRESS("swto23"));
   AddExit("east",   SIL_FORTRESS("swto22"));
