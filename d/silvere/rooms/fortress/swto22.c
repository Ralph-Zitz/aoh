...
varargs void create()
{
   ::create();
   Set(P_INDOORS,1);
   Set(P_INT_SHORT,"Entrance to the southwestern tower");
   Set(P_INT_LONG,
    ...
    "high and two doors lead out, one to the southern battlement and the "
    "other one to the western battlement. There's also a staircase leading "
    "up to the top of the tower and down to the tower lower level.\n");

   Set(P_INT_WIZMSG,
    " South: SE Corner of SW tower, level 2.\n"
    " East:  Enter the southern battlement.\n"
    " North: Enter the western battlement.\n"
    " West:  NW Corner of SW tower, level 2.\n"
    " Up:    Top of the SW tower, level 3.\n"
    " Down:  Bottom of the SW tower, level 1.\n");
...
//   Set(P_COORDINATES, ({ ({ -50,20,0 }), C_SILVERE }) );
   ...
   AddExit("north",  SIL_FORTRESS("webat8"));
   AddExit("south",  SIL_FORTRESS("swto24"));
   AddExit("east",   SIL_FORTRESS("sobat1"));
   AddExit("west",   SIL_FORTRESS("swto21"));
   AddExit("up",     SIL_FORTRESS("swto34"));
   AddExit("down",   SIL_FORTRESS("swto14"));
