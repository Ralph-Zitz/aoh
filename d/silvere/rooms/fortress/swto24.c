...
string CheckBatt()
{
   ...
   switch(({int})NIGHTDAY->QueryState())
   {
      ...
      case ND_TWILIGHT..ND_EVENING:
         desc+=
          "you see the shadows starting to cover the battlements and "
          "the gatehouse shrouding them in darkness.\n";
         break;
      case ND_EARLY_MORNING..ND_SUNSET:
         desc+=
...
varargs void create()
{
   ::create();
   Set(P_INDOORS,1);
   Set(P_INT_SHORT,"Southeastern corner of southhwestern tower");
   Set(P_INT_LONG,
   ...
   Set(P_INT_WIZMSG,
    " North: Entrance to northwestern tower, level 2.\n"
    " West:  SW Corner of SW tower, level 2.\n");
   ...
   AddDetail( ({"slit","slits","arrow slit","arrow slits"}),
    "The arrow slits are narrow and tall letting in a bit of wind and "
    "light into the dimly lighted tower. Peeking outside you take a "
    "a glance of the southern battlement, the fields that surround the "
    "fortress and the river to the south.\n");

   AddDetail( ({"field","fields"}),
    "Peeking south from the arrow slits you can see fields stretch to the "
    "river that flows across them.\n");

   AddDetail( ({"battlement","battlements","south battlement",
   "southern battlement"}),
    "Looking east from the arrow slits the south battlements can be seen, "
    "they are made from stone and offer a strong cover against arrows "
    "shoot from the ground below.\n");
...
//   Set(P_COORDINATES, ({ ({ -50,20,0 }), C_SILVERE }) );
   ...
   AddExit("north",  SIL_FORTRESS("swto22"));
   AddExit("west",   SIL_FORTRESS("swto23"));
