// This is a small place in the forest, where you can find two rats.
      ...
      for(a=0;a<sizeof(exitkey); a++)
      {
         if(exitkey[a]=="east")
         {
            ...
            if(present("rat",find_object(exitdesc[a])))
               desc+="Something squeaks to the east. ";
         }
      }
      desc+="\n";
   }
...
"Some rats are here. Just some nice and easy creatures to kill newbies\
 ...
 Southwest: To a crossing.\n\
 East:      Follow the road.\n");

   Set(P_INT_LONG,
      "The path leads in from the southwest and continues to the east. "
   ...
   Set(P_COORDINATES, ({ ({-130,70,0 }), C_SILVERE }) );

   Set(P_NOWAY_MSG,
"You bump into a tree like an idiot. If you do such things often, you\
 probably need a new face in a not too far future.\n");

   AddExit("southwest", SIL_PARK("forest_room5"));
   AddExit("east",      SIL_PARK("forest_north2"));

   AddItem("/std/npc",REFRESH_HOME,
   ({
      ([ P_LEVEL:1,
         P_LONG:
"This is a small, ugly rat. It looks at you and hisses angrily. It "
