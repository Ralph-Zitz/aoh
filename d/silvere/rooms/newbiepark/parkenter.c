...
int leave_park(string str)
{
   ...
   if(strstr(lower_case(str),"park")!=-1)
   {
      ...
      TP->move(SIL_WEST("cityenter"),M_GO,"north");
      return 1;
   }
   return 0;
}
...
int check_ground(string str)
{
   ...
   if(strstr(lower_case(str),"ground")!=-1)
   {
      msg_room(TO,CMSG_ROOM, CAP(NAME(TP))+
         " searches the ground.\n", ({TP}) );
      msg_write(CMSG_GENERIC,"Sorry, "+CAP(RNAME(TP))+", but as "
         "everything here is under construction, you find nothing "
...
"There should be an important looking sign, which gives some\
 instructions on how to fight, how to use the wimpy mode and\
...
East:  walk deeper into the citypark\n");
   ...
   AddDetail( ({"sign","important sign"}),
      ...
      "on it's wooden surface.\n");
   AddDetail( ({"words","word","text"}),
      "The text, which is written on the sign seems to be important. "
   ...
   Set(P_COORDINATES, ({ ({ -180,60,0 }), C_SILVERE }) );

   Set(P_NOWAY_MSG,
"You walk into the wall like an idiot. Somehow you have the feeling, that\
 everyone laughs at you.\n");

   AddExit("north",SIL_WEST("cityenter"));
   AddExit("east", SIL_PARK("forest_enter"));
}
