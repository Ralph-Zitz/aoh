...
int leave_guild(string str)
{
      ...
      strstr(lower_case(str),"building")!=-1)
   {
      ...
      TP->move(SIL_WEST("libraryroad1"),M_GO,"west");
      return 1;
   }
   return 0;
}
...
varargs void create()
{
   ...
   Set(P_INT_LONG,
"This is the entrance to the adventurers guild. To the west you can\
 leave the building to the library road.\n");

   Set(P_INT_WIZMSG,
" WEST:  (OR LEAVE) the road to the library\n");

   /*
    * Add some details here. *grin*
    */

   AddRoomCmd("leave", #'leave_guild /*'*/);

   Set(P_INT_MAP,"silvere1");
   Set(P_REGION,REGION_CITY);
   Set(P_COORDINATES, ({ ({ -90,0,0 }), C_SILVERE }) );

   Set(P_NOWAY_MSG,
"You walk into the wall like an idiot. Somehow you have the feeling, that\
 everyone laughs at you.\n");
   AddExit("west",   SIL_WEST("libraryroad1"));
}
