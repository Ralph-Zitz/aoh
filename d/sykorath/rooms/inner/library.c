...
create()
{
  ...
  SetIndoors(1);
  SetCoordinates( ({ ({560,540,0}), C_SYKORATH }));
  ...
  /* read the index file and put all lines into the array */
  for (i=1;;i++) {
  ...
  Set(P_INT_WIZMSG,
    "%^RED%^replace the windows by vwindows, add details and replace "
    "old details by vitems!%^RESET%^\n"
