 ...
 *  lib.c
 *
 *  THU:19.07.92  THU=Orcgrimm@Nightfall, Archwizard of Finance
 *
 * Heavily rebuilt: Sique April 18 1994
 */
...
{
  return Pvaluta||({string *})MONEY_DEMON->QueryCoinTypes(getuid())||
    ({string *})MONEY_DEMON->QueryStandard();
}
...
varargs int SetMoney(object who, mixed arg, string *types) {
  ...
  if(intp(arg)) return ({int})ob->SetMoney(({mixed})MONEY_DEMON->CreateMoney(arg, types));
...
varargs int QuerySomeonesValue(object who, string *types) {
  ...
  return ({int})MONEY_DEMON->QueryMoneyValue(({mixed})ob->QueryMoney(),types);
}
/*---------------------------------------------------------------------------*/
int PayMoney(object who, mixed m_array) {
  ...
  if(!who) return 0;
  if(!(ob = present(MONEY_NAME,who))) return 0;
  if (({int})ob->QueryValue()<({int})MONEY_DEMON->QueryMoneyValue(m_array))
    return 0;
  m_a = ({mixed})MONEY_DEMON->PayMoney(({mixed})ob->QueryMoney(),m_array);
  if(!m_a) return 0;
  ob->SetMoney(m_a);
  return 1;
}
...
varargs int PayValue(object who, int value, string *types)
{
  ...
  m_a = ({mixed *})MONEY_DEMON->PayValue(value,({mixed})ob->QueryMoney(),types);
  if(!m_a) return 0;
...
varargs mixed PayValue2(object who, int value, string *types)
{
  ...
  ob->SetMoney(m_a[0]);
  if(!({int})MONEY_DEMON->QueryMoneyValue(m_a[1])) ret = ({0});
  else ret=({({string})MONEY_DEMON->QueryMoneyString(m_a[1])});
  if (!({int})MONEY_DEMON->QueryMoneyValue(m_a[2])) ret+=({0});
...
{
  return ({mapping})MONEY_DEMON->CreateMoney(value, types);
}
...
private mixed parse_single(string money, mapping all_money)
{
  ...
  if(!all_money[coin] && !(coin = ({string})MONEY_DEMON->PresentMoney(coin, all_money)))
    return 0;
...
mapping CreateMoneyFromString(string str)
{
  ...
  all_money = map_indices(({mapping})MONEY_DEMON->QueryCoinTypes(),#'true /*'*/);
