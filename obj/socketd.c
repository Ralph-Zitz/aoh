...
#pragma no_inherit, no_shadow, no_range_check

#include <secure/wizlevels.h>
#include <driver/erq.h>
...
public int Write(mixed data) {
  ...
  if (sockets[so_fd, S_OPTS] & SOCKET_MUDMODE) {
    ...
    data = to_array(data)[0..sizeof(data)-1];
    size = sizeof(data);
...
public object New(int type, mixed host, int port, closure cb, int opts) {
  ...
  if (!valid_socket(cb_o))
    RAISE_ERROR(sprintf("privilege violation: New() - cb_o: %O\n", cb_o));
...
int socket_write(int ufd, mixed msg, int* udp_addr) {
  int size, chunk, *ticket, pos, is_udp, fd;
  string snipsel;
  mixed* data;
  ...
  if (pointerp(msg)) {
    ...
    for (; pos < size; pos += chunk) {
      ...
      sendbuf(ufd, udp_addr + to_array(snipsel)[0..sizeof(snipsel)-1], pos);
      ++sockets[fd, S_TO_WRITE];
    }
  }
...
static void lookup(string host, closure cb) {
  ...
  send_erq(ERQ_LOOKUP, to_bytes(host), cb);
}
...
static void debug(varargs mixed* xx) {
  ...
  if (!debugger) return;
#if 0
    if (!debugger = find_player("nostradamus"))
      return;
#endif
