...
nosave string *detail;      /* contains the descripted details */
nosave mixed *desc;         /* contains the lines of the detail descriptions */
...
static void show_detail()
{
  ...
  i = member(detail, QueryLabel("detail"));
  dummy = desc[i];
...
static void show_all_details(int mode)
{
  ...
  for(i=0;i<sizeof(detail);i++)
  {
    if(mode) write((i+1) + ". ");
    write(detail[i] + " ... ");
    description = implode(explode(arrayprint(desc[i]),"\n")," ");
    if(sizeof(description) >
      SCREEN_WIDTH - 5 - (i>9?4:3)*mode - sizeof(detail[i]))
...
static void insert_detail(string str)
{
  ...
  if(i != -1)
  {
    ...
    write(detail[i] + " ... ");
    description = implode(explode(arrayprint(desc[i]),"\n")," ");
    if(sizeof(description) > 74 - sizeof(detail[i]))
      write(description[0..(70 - sizeof(detail[i]))] + "...\n");
    else write(description + "\n");
...
static void insert_detail2(string str)
{
  ...
  desc += ({desc[member(detail, QueryLabel("detail"))]});
  write("Do you want to add a detail with the same description? (y/n) ");
  input_to("same_desc");
}
