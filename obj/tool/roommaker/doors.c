...
nosave string *door;    /* contains the door commands */
nosave string *dpath;   /* contains the door paths */
nosave string *dshort;  /* contains the door short descriptions */
nosave mixed *dlong;   /* contains the door long descriptions */
nosave string *dkey;    /* contains the key ids for the doors */
nosave int    *dstatus; /* contains the door status */
nosave string *statlist; /* contains the possible door stats */
...
mixed Set(string prop, mixed arg);
...
static void door_redisplay2()
{
  int i;
  string dummy;
  dummy = dpath[member(door,  QueryLabel("door"))];
  dpath[member(door, QueryLabel("door"))] = "<no path>";
  if(sizeof(door) == 1) write("This door is");
  else write("These doors are");
  write(" given:\n");
  for(i=0;i<sizeof(door);i++)
    write(door[i] + " ... " + dpath[i] + ", \"" + dshort[i] + "\"\n");
  dpath[member(door, QueryLabel("door"))] = dummy;
  gotogetstring();
}
...
{
  dpath[member(door, QueryLabel("door"))] = str;
  return getdshort();
}
...
static void door_redisplay3()
{
  int i,j;
  string dummy;
  dummy = dshort[j = member(door, QueryLabel("door"))];
...
{
  dshort[member(door, QueryLabel("door"))] = str;
  return getdlong();
}
static void getdlong()
{
...
static void show_door()
{
  ...
  dummy = dlong[i = member(door, QueryLabel("door"))];
...
static void store_dlong()
{
  ...
  dlong[i = member(door, QueryLabel("door"))] = EditText();
  return getdkey();
}
...
{
  if(!str) str = "0";
  dkey[member(door, QueryLabel("door"))] = str;
...
"1 ....... door is per default open   2 ....... door is per default closed\n");
    if(dkey[member(door, QueryLabel("door"))] != "0")
  ...
  if(str == "closed" || str == "2")
  {
    dstatus[member(door, QueryLabel("door"))] = 1;
    return enter_door();
  }
  if(str == "locked" || str == "3"
     && dkey[member(door, QueryLabel("door"))] != "0")
  {
    dstatus[member(door, QueryLabel("door"))] = 2;
    return enter_door();
  }
