...
nosave string *exit;        /* contains the exit commands */
nosave string *path;        /* contains the exit paths */
...
static void exit_redisplay2()
{
  int i;
  string pathdummy;
  pathdummy = path[member(exit,QueryLabel("exit"))];
  path[member(exit,QueryLabel("exit"))] = "<no path>";
  if(sizeof(exit) == 1) write("This exit is");
  else write("These exits are");
  write(" given:\n");
  for(i=0;i<sizeof(exit);i++)
    write(exit[i] + " ... " + path[i] + "\n");
  path[member(exit,QueryLabel("exit"))] = pathdummy;
  gotogetstring();
}
...
{
  path[member(exit,QueryLabel("exit"))] = str;
  return enter_exit();
}
