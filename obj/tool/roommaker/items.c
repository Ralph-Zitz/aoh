...
nosave mixed *item;         /* contains the item settings */
...
{
  item[0] += ({this_player()->get_path(str)});
  item[1] += ({0});
  gotogetmode();
}
...
static void delete_item()
{
  ...
  write(" given:\n");
  for(i=0;i<sizeof(item[0]);i++)
      write((i+1) + ". " + item[0][i] + " ... " + refresh[item[1][i]] + "\n");
  write("Which item should be deleted?\n");
