...
** to get a guildobject fully compatible to the /p/guilds/std/guild.
**
...
** 19feb97 [r] Moved the File to /p/guilds/std/guildobj
** 26apr97 [e] Added command Guildmail
** 26apr97 [e] Added guild maintainers
**
** [t] Thragor
** [r] Rover
** [e] Eilan
...
#define SF(x) #'x //'
#define MYSELF "/p/guilds/std/guildobj"
#define MEMBERS

inherit "/obj/lib/skills";

private static string owner,guild,guildlog;
private static object guildob;
private static string gtploc;
...
string s,tmp;
  ...
  if (guildob->is_creator(pl) && guildob->is_maintainer(pl))
     tmp=" -MAIN/CREATOR-";
  else if (guildob->is_creator(pl)) tmp=" -CREATOR-";
  else if (guildob->is_maintainer(pl)) tmp=" -MAINTAINER-";
  else if (guildob->is_leader(pl)) tmp=" -LEADER-";
  else tmp="";

  s=capitalize(getuid(pl))+" "+
...
protected status who_sort(object pl1,object pl2)
{
  ...
  if (LoadGuild())
  {
      ...
      m1=guildob->is_maintainer(pl1);
      m2=guildob->is_maintainer(pl2);
  }
  if ( (m1&&c1) && ( ! (m2&&c2) ) ) return 1;
  else if ( (m2&&c2) && ( ! (m1&&c1) ) ) return 0;
  else if ( (m1||c1) && ( ! (m2||c2) ) ) return 1;
  else if ( (m2||c2) && ( ! (m1||c1) ) ) return 0;
  else if (m1 && c2) return 1;
  else if (c1 && m2) return 0;
  else if (c1 || m1)
  {
    if ((lvl1=!!query_user_level(pl1))==(lvl2=!!query_user_level(pl2)))
	   return getuid(pl1)>getuid(pl2);
    else return lvl1>lvl2;
  }
  else
  {
...
void input_mail(string str)
{
  string *names;
  string sender;
  mixed *msg;
  if (!guildob) LoadGuild();
  if (!guildob)
  {
    write("Cannot load guild...error.\n");
	return ;
  }
  names=guildob->QueryMemberList();

  if (!str || str=="")
  {
    write("Aborting guild mail.\n");
	return ;
  }
  write("Sending mail to: "+implode(names,", ")+".\n");
  sender=this_player()->QueryRealName();
  names-=({sender});


  msg=({sender,sender,sender,names,({}),mail_subject,0,0,str});
  MAILER->DeliverMail(msg);
}
void input_subject(string str)
{
  if (!str || str=="")
  {
    write("Aborting guild mail.\n");
  ...
  mail_subject="Guildmail: "+str;
  "/lib/string"->input_text("input_mail");
}

public int cmd_gmail(string str)
{ 
  mixed *mesg;
  string *delivered_to;
  string name,s,subject,text;
  string *slist;
  int n,i;
  ...
  if (!TP) return 0;
  if (str) 
    return notify_fail("You cannot use an argument here.\n",NOTIFY_NOT_OBJ),0;
  if (!LoadGuild())
...
{
  check_owner(environment()||this_player());
}
...
public int cmd_skills(string str)
{
  ...
  if (str)
    return (int)notify_fail("Just type '"+query_verb()+"'\n",
      NOTIFY_ILL_ARG)&&0;
  if (!LoadGuild())
    return (int)notify_fail("The guild can't be loaded.\n"
       "Inform the maintainer of the guild.\n",NOTIFY_NOT_VALID)&&0;
  if (!sizeof(skill_list = m_indices(guildob->QuerySkills(TP)||([]))))
    return (int)notify_fail("You have no skills.\n",
      NOTIFY_NOT_VALID)&&0;
