...
make7(arg) {
  if (arg=="~q") return 1;
  cert_file=arg;
  if ("/p/houses/common/room/proppool"->AddProperty(house_dir, exit, road, to_int(price), description, cert_file) == 1) write("Ok.\n");
  return 1;
}
make6(arg) {
  if (arg=="~q") return 1;
...
The standard certfile is located in /p/houses/common/room/cert_dummy\n\
Certificate file: ");
  input_to("make7");
}
make5(arg) {
