// This is a standard spellmonster which provides easy to use witches
// spells. One need only 2-3 commands to have a full NPC witch
// 
// [s] 10aug96 creation
//
// [s] Softbyte
...
/* ------------------------------------------------------------------ */
public mapping spell_frog(mapping sp)
{
  object *en,vic,env;
  int i;
  ...
  en = (QueryEnemies()||({}))+({});
  en = filter(en,#'valid_victim);
  en-= filter_objects(en,"QueryFrog");
  if (!sizeof(en)) return 0;
  vic = en[random(sizeof(en))];
  tell_object(vic,parse_string(
              "@@MyName|1@@ makes some strange looking gestures. You "
              "feel somehow smaller and greenish.. you think a fly would "
              "be tasteful.\n"));
  show_room(env,parse_string("@@MyName|1@@ makes some strange looking "
                             "gerstures"+capitalize(vic->QueryName())+
                             " looks like a frog now.\n"),
            ({vic}));
  vic->SetFrog(1);
  return sp;
}
