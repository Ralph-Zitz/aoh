...
varargs void create() 
{
  // Here we call the create() in the parent object, which is here /std/room.
  ...
  Set(P_INT_SHORT, "The first example room");
    ...
    ** The various strings are collated by the gamedriver into one string.
    ...
    */
  Set(P_INT_LONG,
   "You are in the first example room (/d/archwiz/common/lpc/exm/xroom1.c).\n"
   "It features a 'normal' exit south, 'special' exits west and southwest, "
   "and a hidden exit.\n"
   "Note that the obvious exits are here always shown.\n"
  ...
  CheckExit("southwest", DACT_CHECK);
    ...
    /* The the southeast is a hidden exit into the xthingroom.
     */
