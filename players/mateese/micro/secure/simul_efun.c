...
// TODO: command-Stackhandling, including notify_fail(), command()
...
//---------------------------------------------------------------------------
public nomask varargs mixed * slice_array(mixed * arr, int from, int to)
{
  raise_error("* Outdated efun slice_array() used.\n");
  return ({});
}

//---------------------------------------------------------------------------
public nomask varargs string extract(string str, int from, int to)
{
  raise_error("* Outdated efun extract() used.\n");
  return "";
}
...

{
  ...
  if (!(obj || (obj = PREV)))
    return 0;
  i = obj->Query(SP_Cloned);
  if (i)
    return i > 0;
  return (query_once_interactive(obj) || member(file_name(obj), '#') > 0);
}
...
// Tell a player a text if he is online.
//
//   whom: name of the player to tell the string.
//   what: the text to tell.
//
// The function is useful for debugging.
...
// TODO: Special snoop free rooms. General notified snoops. Snooper is stored
//   in the snoopee.
