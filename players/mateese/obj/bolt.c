...
/*-------------------------------------------------------------------------
** Move one room into the desired direction. 
** Return 0 on failure, 1 on success, 2 if a door was pierced through.
*/
  ...
  if (aim_data) {
    ...
    if (!(nextroom = assoc (dir, exits))) {
      ...
      if (data = assoc (dir, exits)) {
        if (data[DOOR_STATUS] && !ack_doors) rc = 2;
        if (!ack_doors || !data[DOOR_STATUS])
...
void _explode_door (mixed *data) {
  object lastroom, thisroom;
  int lastdix, thisdix;
  string lastdkey, thisdkey;
  mixed *lastdoor, *thisdoor, *doors;

  lastroom = data[0]; lastdix = data[1]; 
  lastdkey = data[2]; lastdoor = data[3];
  thisroom = data[4]; thisdix = data[5]; 
  thisdkey = data[6]; thisdoor = data[7];
  show_room (environment(),
...
/*-------------------------------------------------------------------------
** Hit a door.
** Doors are assumed to have an resistance equivalent to 10kg.
** Higher energized bolts just pierce through, the medium energized ones
** cause structural damage (or destroy the door), the others just
** disappear.
** Return 1 if the bolt is absorbed by the door in any way.
*/

int hit_door (object lastroom) {
  int doordamage, lastdix, thisdix;
  string lastdkey, thisdkey;
  mixed *lastdoor, *thisdoor, *doors;
  int rm_me;

    /* Get doorinfos of both rooms */
  lastdkey = dir; 
  lastdoor = assoc (dir, (mixed *)lastroom->QueryDoors());
  lastdix = assoc (dir, ((mixed *)lastroom->QueryDoors())[0]);
  doors = (mixed *)environment ()->QueryDoors();
  if (sizeof(doors[0])) {
    thisdkey = lower_case(lastdoor[DOOR_SHORT]);
    thisdix = sizeof(doors[0]);
    while(thisdix--) {
      if (lower_case(doors[1][thisdix][DOOR_SHORT]) == thisdkey) {
        thisdoor = doors[1][thisdix];
        thisdkey = doors[0][thisdix];
        break;
      }
    }
  }
  doordamage = (int)lastroom->Query("_PlasmaDoor"+lastdkey);
  if (energy * 500 >= 10000 * 2) { /* Pierce through */
    doordamage++;
    show_room (lastroom, 
               ({(distance?"A":"The")+" plasma bolt pierces through "+
                 lastdoor[DOOR_SHORT]+".\n",
                "A plasma bolt howls through the room, heading "+dir+".\n"})
              );
    show_room (environment(), 
               "A plasma bolt pierces through "+thisdoor[DOOR_SHORT]+".\n");
    energy -= 10000/500;
  }
  else if (energy * 500 >= 10000 / 4) { /* Damage door */
    doordamage += energy / 2;
    show_room (lastroom, 
               ({(distance?"A":"The")+" plasma bolt impacts into "+
                 lastdoor[DOOR_SHORT]+".\n",
                 "A plasma bolt "+(distance ? "howls in and ":"")+
                 "dies in a heavy impact.\n" })
              );
    tell_room (environment(), 
               "You hear a loud noise from "+thisdoor[DOOR_SHORT]+".\n");
    rm_me = 1;
  }
  else { /* Door withstands the impact */
    show_room (lastroom,
               ({(distance?"A":"The")+" plasma bolt is absorbed by "+
                 lastdoor[DOOR_SHORT]+".\n",
                 "A plasma bolt "+(distance ? "howls in and ":"")+
                 "is absorbed by something solid.\n" })
              );
    rm_me = 1;
  }
  if (doordamage >= 10000 / 500) { /* Door is destroyed */
    SetShort (0);
    SetBright (0);
    call_out ("_explode_door"
              ({ lastroom, lastdix, lastdkey, lastdoor,
                 environment(), thisdix, thisdkey, thisdoor }));
    rm_me = 1;
  }
  else if (rm_me) remove();
  return rm_me;
}
...
int hit_object (object obj) {
  ...
  hits = (int) obj->Query("_PlasmaHits");
  obj->Set("_PlasmaHits", hits+1);
  if (!hits) {
...
void _fire () {
  ...
  do {
    ...
    lastroom = environment();
      ...
      /* Is our target here? If yes, try to hit it. 
      ** The bolt has a minimal accuracy of 10%, and a maximal of 99%.
      ** The first half of the distance accuracy lowers slowly
      ** downto 90%, then fast.
      */
    if (target && present (target, environment())) {
      int accuracy;
      if (distance <= MAX_DISTANCE/2)
        accuracy = 990 - ((180 * distance) / MAX_DISTANCE);
      else
        accuracy = 1700 - ((1600 * distance) / MAX_DISTANCE) + 800;
      if (random(1000) <= accuracy && hit_object (target)) return;
    }

    if (!dir || !(rc  = _move_on (dir, 0))) {
      hit_room (lastroom);
      return;
    }
    if (rc == 2) 
      if (hit_door (lastroom)) return; else;
    else if (distance)
      tell_room (lastroom, 
