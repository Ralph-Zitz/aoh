...
static object find_item (string item, object prev) {
  ...
  if (item == "here") return environment(PL);
  if (item == "me"  ) return PL;
  ...
  if (mode) {
    ...
    while (ob) {
      if (mode == 1) {
        if (sscanf (" "+ob->QueryShort()+" ", str, foo, bar) == 2) return ob;
      }
      if (mode == 2) {
...
varargs object find (string items, object prev) {
  ...
  items = ""+items;
  if (!prev) mark = environment(PL); else mark = prev;
