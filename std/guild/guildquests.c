...
private nosave mapping guildquests;
...
// another guildquest
{
  ...
  if (!pl) return 0;
  if (sizeof(h = m_indices(({mapping})ME->QueryLevelCosts()))==0)
    return 0;
  else
    maxlvl = ({int})NMB->max(h);
  ids = m_indices(guildquests||([]));
  ids-=({string *})QUESTMASTER->QueryPlayerQuests(pl);
  if (!sizeof(ids)) return maxlvl;
  qlvls = map(ids,LSF("GetQuestLevel"));
  return (({int})NMB->min(qlvls))-1;
}
    ...
    for(j=0;j<sizeof(res[h[i]]);j++)
      {
        ...
        list+=sprintf("%2d. %s (level %d)",(i+j+1),
          ...
          ((({int})QUESTMASTER->QueryQuestSolved(id,pl))?" (solved)":"")+"\n";
        nres[i+j+1] = id;
      }
  return list;
}
