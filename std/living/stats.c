...
int dispatchtalent(string str) {
    ...
    for (i=0;i<sizeof(cmds);i++) {
        ...
        if (!member(Talents,name)) {  // remove entry on the fly
            ...
            if (tmp)
                flag=tmp;  // someone returned TRUE
        }
    }
    return flag;
}
...
void AddTalentCommands(string name,int value,string filename,mixed extradata) {
    ...
    for (i=0;i<sizeof(cmds);i++) {
        ...
        if (-1!=member( ({"tell","say","who","score","ls","cd","pwd",
            ...
            log_file("living.log",
                    (ob?(({string})ob->QueryRealName()||"Nobody"):"None")+
                    " added talent command '"+cmds[i]+"' from file '"+
                    file+"' for living '"+
...
varargs int SetTalent(string name,int value,string filename,mixed extra) {
    ...
    if (member(Talents,name)) { // Change talent
        ...
        Talents+=([name:value;filename;extra]);
    }
...
// Debugging only
mapping QueryMTalents() {
    return Talents;
}

string *QueryDefTalents() {
    return defTalents;
}
...
public void init() {
    if (Talents && sizeof(Talents))
        walk_mapping(Talents,#'InitTalent /*'*/);
}

public varargs int remove(int arg) {
    if (Talents || sizeof(Talents) )
        walk_mapping(Talents,#'RemoveTalent /*'*/);
    Talents=([]);
    return 0;
}
