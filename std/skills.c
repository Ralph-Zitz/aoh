...
varargs int AddSkill(string skill,string name,int difficulty,mixed ids,mixed props)
{
  ...
  if (old_map)
  {
    ...
    if (member(old_map,name)) old_map=m_delete(old_map,name);
...
string MatchIds(string *ids,string str)
{
  ...
  for (i=0;i<sizeof(ids);i++)
	{
	  if (strstr(str,ids[i])==0) return ids[i];
	}
	return 0;
}
...
int cmd_skills(string str)
{
  ...
  if (!TP || !skill_verb) return 0;

  if (str) str=norm_id(str);
...
//    str,id_str||"NULL",extra_str||"NULL");
  ...
  if (found_p<0 && null_p>=0) found_p=null_p;
